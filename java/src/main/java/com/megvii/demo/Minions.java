package com.megvii.demo;

import org.apache.commons.httpclient.HttpClient;
import org.apache.commons.httpclient.methods.PostMethod;
import org.apache.commons.httpclient.methods.multipart.FilePart;
import org.apache.commons.httpclient.methods.multipart.MultipartRequestEntity;
import org.apache.commons.httpclient.methods.multipart.Part;
import org.apache.commons.httpclient.methods.multipart.StringPart;
import org.apache.commons.httpclient.params.HttpMethodParams;
import org.json.JSONObject;

import java.io.*;
import java.util.HashMap;
import java.util.Map;

/**
 * Created by wushengping on 2019/10/21.
 */
public class Minions {
    public static void main(String[] args) {
        minionsVerify("http://10.201.102.109:9000/verify");
        minionsCompare("http://10.201.102.109:9000/compare");
        minionsExtractWithDetect("http://10.201.102.109:9000/extract_with_detect");
    }

    public static void minionsVerify(String url) {
        try {
            //todo: 更多请求参数请查看api文档
            FilePart bestPart = new FilePart("image_idcard", new File("a.jpg"));
            FilePart idcardPart = new FilePart("image_best", new File("a.jpg"));
            StringPart stringPart = new StringPart("rotate", "1");
            Part[] parts = new Part[]{bestPart, idcardPart, stringPart};
            String result = HttpUtils.postWithFormData(url, parts);
            System.out.print(result);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    }

    public static void minionsExtractWithDetect(String url) {
        try {
            //todo: 更多请求参数请查看api文档
            FilePart imagePart = new FilePart("image", new File("a.jpg"));
            StringPart stringPart = new StringPart("cutimg", "1");
            Part[] parts = new Part[]{imagePart, stringPart};
            String result = HttpUtils.postWithFormData(url, parts);
            System.out.print(result);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    }

    public static void minionsCompare(String url) {
        String feat = "";
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("feat1", feat);
        jsonObject.put("feat2", feat);
        String result = HttpUtils.postJson(url, jsonObject.toString());
        System.out.print(result);
    }
}

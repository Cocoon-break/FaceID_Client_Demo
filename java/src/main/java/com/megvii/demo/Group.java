package com.megvii.demo;

import org.apache.commons.httpclient.methods.multipart.Part;
import org.apache.commons.httpclient.methods.multipart.StringPart;

/**
 * Created by wushengping on 2019/10/21.
 */
public class Group {
    public static void main(String[] args) {
        groupInit("http://10.201.102.247:9090/group/init");
        groupAdd("http://10.201.102.247:9090/group/add");
        groupSearch("http://10.201.102.247:9090/group/search");
    }
    public static void groupInit(String url){
        StringPart stringPart = new StringPart("groupname", "test");
        Part[] parts = new Part[]{stringPart};
        String result = HttpUtils.postWithFormData(url, parts);
        System.out.print(result);
    }
    public static void groupAdd(String url){
        String feat = "";
        StringPart groupName = new StringPart("groupname", "test");
        StringPart feature = new StringPart("feature", feat);
        Part[] parts = new Part[]{groupName,feature};
        String result = HttpUtils.postWithFormData(url, parts);
        System.out.print(result);
    }
    public static void groupSearch(String url){
        String feat = "";
        StringPart groupName = new StringPart("groupname", "test");
        StringPart feature = new StringPart("feature", feat);
        Part[] parts = new Part[]{groupName,feature};
        String result = HttpUtils.postWithFormData(url, parts);
        System.out.print(result);
    }
}
